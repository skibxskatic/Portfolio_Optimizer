# Fidelity Portfolio Optimization Report

**Generated on:** March 05, 2026 at 09:29 PM

> **Privacy Note:** This report was generated entirely locally. Financial quantities and dollar amounts were NOT transmitted to the cloud AI.

## 1. High-Level Metrics
- **Weighted Average Expense Ratio:** `0.033%`
  - ✅ *Excellent: Your portfolio fees are highly optimized.*

## 2. Asset Holding Breakdown
| Symbol | Account Name | Description | Current ER | Suggested Action |
|---|---|---|---|---|
| FDLXX | Cash Management (Individual - TOD) | FIDELITY TREASURY ONLY MONEY MARKET FD | 0.000% | Core Cash Position |
| SPAXX | Cash Management (Individual - TOD) | HELD IN MONEY MARKET | 0.000% | Core Cash Position |
| DIDIY | DIDIY | DIDI GLOBAL INC SPON ADS EACH REP 0.25 CL A ORD SHS | 0.000% | Keep |
| FBGRX | Health Savings Account | FIDELITY BLUE CHIP GROWTH | 0.610% | **Replace (High ER)**. See *Alternatives* below. |
| FDRXX | Health Savings Account | HELD IN MONEY MARKET | 0.000% | Core Cash Position |
| FNCMX | Health Savings Account | FIDELITY NASDAQ COMPOSITE INDEX | 0.290% | Keep |
| FNILX | Health Savings Account | FIDELITY ZERO LARGE CAP INDEX FUND | 0.000% | Keep |
| FTEC | Health Savings Account | FIDELITY MSCI INFORMATION TECHNOLOGY INDEX ETF | 0.084% | Keep |
| SPYG | Health Savings Account | SPDR SERIES TRUST STATE STREET PORTFOLIO S&P 500 GROWTH ETF | 0.040% | Keep |
| SPYM | Health Savings Account | SPDR PORTFOLIO S&P 500 ETF | 0.020% | Keep |
| SPYV | Health Savings Account | SPDR SERIES TRUST STATE STREET PORTFOLIO S&P 500 VALUE ETF | 0.040% | Keep |
| XLK | Health Savings Account | SELECT SECTOR SPDR TRUST STATE STREET TECHNOLOGY SELECT SECTOR SPDR ETF | 0.080% | Keep |
| SPAXX | INDIVIDUAL | HELD IN MONEY MARKET | 0.000% | Core Cash Position |
| AAPL | INDIVIDUAL | APPLE INC | 0.000% | Keep |
| DELL | INDIVIDUAL | DELL TECHNOLOGIES INC CL C | 0.000% | Keep |
| DGRO | INDIVIDUAL | ISHARES TRUST CORE DIVID GWTH | 0.080% | Keep |
| FELG | INDIVIDUAL | FIDELITY ENHANCED LARGE CAP GROWTH ETF | 0.180% | Keep |
| FNILX | INDIVIDUAL | FIDELITY ZERO LARGE CAP INDEX FUND | 0.000% | Keep |
| FTEC | INDIVIDUAL | FIDELITY MSCI INFORMATION TECHNOLOGY INDEX ETF | 0.084% | Keep |
| SMH | INDIVIDUAL | VANECK ETF TRUST SEMICONDUCTR ETF | 0.350% | Keep |
| VGT | INDIVIDUAL | VANGUARD WORLD FD INF TECH ETF | 0.090% | Keep |
| VTI | INDIVIDUAL | VANGUARD INDEX FDS VANGUARD TOTAL STK MKT ETF | 0.030% | Keep |
| XLK | INDIVIDUAL | SELECT SECTOR SPDR TRUST STATE STREET TECHNOLOGY SELECT SECTOR SPDR ETF | 0.080% | Keep |
| SPAXX | Melissa Investments | HELD IN MONEY MARKET | 0.000% | Core Cash Position |
| DGRO | Melissa Investments | ISHARES TRUST CORE DIVID GWTH | 0.080% | Keep |
| FDVV | Melissa Investments | FIDELITY HIGH DIVIDEND ETF | 0.150% | Keep |
| SCHD | Melissa Investments | SCHWAB US DIVIDEND EQUITY ETF | 0.060% | Keep |
| VTI | Melissa Investments | VANGUARD INDEX FDS VANGUARD TOTAL STK MKT ETF | 0.030% | Keep |
| VYM | Melissa Investments | VANGUARD WHITEHALL FDS HIGH DIV YLD | 0.040% | Keep |
| LEGR | ROTH IRA | FIRST TR EXCHANGE-TRADED FD VI INDXX INOVTV ETF | 0.650% | **Replace (High ER)**. See *Alternatives* below. |
| FDRXX | ROTH IRA | HELD IN MONEY MARKET | 0.000% | Core Cash Position |
| DGRO | ROTH IRA | ISHARES TRUST CORE DIVID GWTH | 0.080% | Keep |
| FELG | ROTH IRA | FIDELITY ENHANCED LARGE CAP GROWTH ETF | 0.180% | Keep |
| FENY | ROTH IRA | FIDELITY MSCI ENERGY INDEX ETF | 0.084% | Keep |
| FNCL | ROTH IRA | FIDELITY MSCI FINLS INDEX ETF | 0.084% | Keep |
| FNILX | ROTH IRA | FIDELITY ZERO LARGE CAP INDEX FUND | 0.000% | Keep |
| FTEC | ROTH IRA | FIDELITY MSCI INFORMATION TECHNOLOGY INDEX ETF | 0.084% | Keep |
| JEPI | ROTH IRA | J P MORGAN EXCHANGE TRADED FD EQUITY PREMIUM | 0.350% | Keep |
| ONEQ | ROTH IRA | FIDELITY NASDAQ COMPOSITE INDEX ETF | 0.210% | Keep |
| SCHD | ROTH IRA | SCHWAB US DIVIDEND EQUITY ETF | 0.060% | Keep |
| SMH | ROTH IRA | VANECK ETF TRUST SEMICONDUCTR ETF | 0.350% | Keep |
| VNQ | ROTH IRA | VANGUARD REAL ESTATE ETF | 0.130% | Keep |
| VYM | ROTH IRA | VANGUARD WHITEHALL FDS HIGH DIV YLD | 0.040% | Keep |

## 3. Tax Optimization & Loss Harvesting
By tracking individual lot purchase dates via FIFO accounting, we can optimize your short-term/long-term capital gains classification and find tax loss harvesting opportunities.

### 🚨 Tax-Loss Harvesting Candidates
The following lots are currently held at a loss. Selling these will harvest the loss to offset your other capital gains (up to $3,000 against ordinary income).

| Symbol | Description | Tax Category | Underwater Lots |
|---|---|---|---|
| **AAPL** | APPLE INC | STCG (<1yr) | 3 lot(s) underwater |
| **FELG** | FIDELITY ENHANCED LARGE CAP GROWTH ETF | STCG (<1yr) | 20 lot(s) underwater |
| **FNCL** | FIDELITY MSCI FINLS INDEX ETF | STCG (<1yr) | 3 lot(s) underwater |
| **FNCMX** | FIDELITY NASDAQ COMPOSITE INDEX | STCG (<1yr) | 2 lot(s) underwater |
| **FNILX** | FIDELITY ZERO LARGE CAP INDEX FUND | STCG (<1yr) | 3 lot(s) underwater |
| **FTEC** | FIDELITY MSCI INFORMATION TECHNOLOGY INDEX ETF | STCG (<1yr) | 48 lot(s) underwater |
| **ONEQ** | FIDELITY NASDAQ COMPOSITE INDEX ETF | STCG (<1yr) | 3 lot(s) underwater |
| **SPYG** | SPDR SERIES TRUST STATE STREET PORTFOLIO S&P 500 GROWTH ETF | STCG (<1yr) | 3 lot(s) underwater |
| **SPYM** | SPDR PORTFOLIO S&P 500 ETF | STCG (<1yr) | 2 lot(s) underwater |
| **VGT** | VANGUARD WORLD FD INF TECH ETF | STCG (<1yr) | 3 lot(s) underwater |
| **VTI** | VANGUARD INDEX FDS VANGUARD TOTAL STK MKT ETF | STCG (<1yr) | 4 lot(s) underwater |
| **XLK** | SELECT SECTOR SPDR TRUST STATE STREET TECHNOLOGY SELECT SECTOR SPDR ETF | STCG (<1yr) | 17 lot(s) underwater |

### ⏳ Capital Gains 'One-Year Wait' Screener
Profitable lots held for under 365 days are subject to your ordinary income tax rate. Waiting 1 year drops this to the much lower LTCG (15-20%) bracket.

| Symbol | Lots Held < 365 Days (STCG - AVOID SELLING) | Lots Held > 365 Days (LTCG - SAFE) |
|---|---|---|
| **DIDIY** | 0 Lots Pending | 0 Lots Safe to Sell |
| **FTEC** | 54 Lots Pending | 21 Lots Safe to Sell |
| **AAPL** | 2 Lots Pending | 5 Lots Safe to Sell |
| **FELG** | 32 Lots Pending | 13 Lots Safe to Sell |
| **FNCL** | 2 Lots Pending | 4 Lots Safe to Sell |
| **SMH** | 14 Lots Pending | 0 Lots Safe to Sell |
| **SPYG** | 2 Lots Pending | 1 Lots Safe to Sell |
| **FNILX** | 66 Lots Pending | 12 Lots Safe to Sell |
| **SPYM** | 3 Lots Pending | 1 Lots Safe to Sell |
| **SCHD** | 8 Lots Pending | 0 Lots Safe to Sell |
| **VGT** | 2 Lots Pending | 0 Lots Safe to Sell |
| **XLK** | 0 Lots Pending | 0 Lots Safe to Sell |
| **VYM** | 4 Lots Pending | 0 Lots Safe to Sell |
| **DGRO** | 17 Lots Pending | 0 Lots Safe to Sell |
| **SPYV** | 5 Lots Pending | 1 Lots Safe to Sell |
| **FENY** | 6 Lots Pending | 0 Lots Safe to Sell |
| **FDVV** | 2 Lots Pending | 0 Lots Safe to Sell |
| **FNCMX** | 10 Lots Pending | 3 Lots Safe to Sell |
| **FBGRX** | 11 Lots Pending | 3 Lots Safe to Sell |
| **LEGR** | 5 Lots Pending | 4 Lots Safe to Sell |
| **VTI** | 3 Lots Pending | 0 Lots Safe to Sell |
| **ONEQ** | 2 Lots Pending | 4 Lots Safe to Sell |
| **VNQ** | 2 Lots Pending | 0 Lots Safe to Sell |
| **JEPI** | 2 Lots Pending | 0 Lots Safe to Sell |
| **DELL** | 1 Lots Pending | 4 Lots Safe to Sell |

## 4. Recommended Replacements (1-3 Year Horizon)
For your goal of **1-3 year high percentage returns with minimal expenses and high dividends**, volatility mitigation via strong cash flow is key. The following funds were dynamically selected today based on live market data, ranking highest in yield and 1-year momentum while strictly keeping Expense Ratios below `0.40%`:

### Tax-Free/Advantaged Accounts (Roth IRA, HSA)
These funds generate high income/dividends. Place them in tax-advantaged accounts to avoid annual tax drag.

| Ticker | Fund Name | Expense Ratio | Yield | 1-Year | 3-Year (Avg) | 5-Year (Avg) | Tax Location Routing |
|---|---|---|---|---|---|---|---|
| **XLE** | State Street Energy Select Sect | `0.08%` | *2.62%* | +25.00% | +12.53% | +21.58% | **Tax-Free (Roth/HSA)** |
| **SCHD** | Schwab US Dividend Equity ETF | `0.06%` | *3.30%* | +15.74% | +11.88% | +10.56% | **Tax-Free (Roth/HSA)** |
| **FDVV** | Fidelity High Dividend ETF | `0.15%` | *2.77%* | +4.19% | +18.33% | +15.24% | **Tax-Free (Roth/HSA)** |
| **VYM** | Vanguard High Dividend Yield ET | `0.04%` | *2.26%* | +8.06% | +15.57% | +13.15% | **Tax-Free (Roth/HSA)** |
| **SPYD** | State Street SPDR Portfolio S&P | `0.07%` | *4.08%* | +10.99% | +10.50% | +9.71% | **Tax-Free (Roth/HSA)** |

### Taxable Brokerage Accounts (Individual)
These funds prioritize growth or broad market stability with low distributions, making them highly tax-efficient to hold long term.

| Ticker | Fund Name | Expense Ratio | Yield | 1-Year | 3-Year (Avg) | 5-Year (Avg) | Tax Location Routing |
|---|---|---|---|---|---|---|---|
| **QQQ** | Invesco QQQ Trust, Series 1 | `0.18%` | *0.46%* | -1.09% | +27.54% | +15.66% | **Taxable (Individual)** |
| **SPY** | State Street SPDR S&P 500 ETF T | `0.09%` | *1.06%* | +0.66% | +20.82% | +14.28% | **Taxable (Individual)** |
| **DGRO** | iShares Core Dividend Growth ET | `0.08%` | *1.96%* | +6.44% | +15.76% | +12.67% | **Taxable (Individual)** |
| **VIG** | Vanguard Div Appreciation ETF | `0.04%` | *1.56%* | +3.63% | +15.63% | +12.47% | **Taxable (Individual)** |
| **IWM** | iShares Russell 2000 ETF | `0.19%` | *0.98%* | +6.21% | +11.39% | +4.73% | **Taxable (Individual)** |

