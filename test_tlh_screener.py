...
def test_de_minimis_threshold():
    ...
    if gain_pct >= DE_MINIMIS_GAIN_PCT:
        print(f"❌ TEST FAILED: VOO gain % ({gain_pct*100:.2f}%) should be below de minimis threshold ({DE_MINIMIS_GAIN_PCT*100:.0f}%)")
        return False
    
    print(f"✅ DE MINIMIS TEST PASSED: VOO gain ({gain_pct*100:.2f}%) is below {DE_MINIMIS_GAIN_PCT*100:.0f}% threshold — safe to reallocate.")
    return True
